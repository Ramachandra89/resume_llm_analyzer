...
PROMPT_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'prompts', 'prompt_2.txt')
    ...
    if submit:
        if not resume_file or not job_desc:
            ...
            if result["status"] == "success":
                st.markdown(result["response"])  # The prompt already includes section headings
            else:
